...
public interface UserMapper {
    ...
    @Insert("create table user(username varchar(32),password varchar(32),name varchar(32),birthday date,idenity char(18),isOnline tinyint(1) default 0,primary key(username))")
    public boolean createTable();

    @Insert("insert into user values(#{username},#{password},#{name},#{birthday},#{idenity},#{isOnline})")
    ...
    @Update("update user set password=#{password},name=#{name},birthday=#{birthday},idenity=#{idenity},isOnline=#{isOnline} where username=#{username}")
